...
* @license MIT
*
* BUILT: 2026-09-15T07:26:02.681Z
*/;
import "@svgdotjs/svg.select.js";
import { Box, Element, Matrix, Point, extend, off, on } from "@svgdotjs/svg.js";

//#region src/ResizeHandler.js
...
var ResizeHandler = class {
	...
	active(value, options) {
		...
		if (this.lastEvent) {
			if (this.eventType === "rot") this.rotate(this.lastEvent);
			else if (this.eventType === "point") this.movePoint(this.lastEvent);
			else this.resize(this.lastEvent);
		}
	}
	handleResize(e) {
		...
		if (e.type === "point") on(window, eventMove, this.movePoint);
		else if (e.type === "rot") on(window, eventMove, this.rotate);
		else on(window, eventMove, this.resize);
	...
	endResize(ev) {
		if (this.eventType !== "rot" && this.eventType !== "point") this.resize(ev);
		this.lastEvent = null;
		this.eventType = "";
	...
	snapToAngle(angle) {
		if (this.degree) angle = Math.round(angle / this.degree) * this.degree;
		return angle;
	}
};

//#endregion
//#region src/svg.resize.js
extend(Element, { resize: function(enabled = true, options = {}) {
	...
	if (!resizeHandler) {
		if (enabled.prototype instanceof ResizeHandler) {
			...
			enabled = true;
		} else resizeHandler = new ResizeHandler(this);
