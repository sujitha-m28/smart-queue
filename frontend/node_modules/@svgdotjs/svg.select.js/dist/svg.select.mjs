...
* @license MIT
*
* BUILT: 2026-09-15T07:21:13.161Z
*/;
import { Element, G, Line, Polygon, Polyline, extend, getWindow } from "@svgdotjs/svg.js";
...
var SelectHandler = class {
	...
	createResizeHandles() {
		this.handlePoints.forEach((p, index, arr) => {
			...
			this.selection.last().addClass("svg_select_handle svg_select_handle_" + name).on("mousedown.selection", getMoseDownFunc(name, this.el, this.handlePoints, index)).on("touchstart.selection", getMoseDownFunc(name, this.el, this.handlePoints, index), void 0, { passive: false });
		});
	}
	...
	createRotationHandle() {
		...
		const handle = this.selection.group().addClass("svg_select_handle_rot").on("mousedown.selection", getMoseDownFunc("rot", this.el, this.handlePoints)).on("touchstart.selection", getMoseDownFunc("rot", this.el, this.handlePoints), void 0, { passive: false });
...
//#endregion
//#region src/PointSelectHandler.js
var PointSelectHandler = class {
	constructor(el) {
		this.el = el;
	...
	createPointHandles() {
		this.points.forEach((p, index, arr) => {
			...
			this.selection.get(index + 1).addClass("svg_select_handle_point").on("mousedown.selection", getMoseDownFunc("point", this.el, this.points, index)).on("touchstart.selection", getMoseDownFunc("point", this.el, this.points, index), void 0, { passive: false });
		});
	}
	createHandleFn(group) {
		group.circle(5);
...
//#endregion
//#region src/svg.select.js
var getSelectFn = (handleClass) => {
	return function(enabled = true, options = {}) {
		...
		if (!selectHandler) {
			if (enabled.prototype instanceof SelectHandler) {
				...
				enabled = true;
			} else selectHandler = new handleClass(this);
...
extend(Element, { select: getSelectFn(SelectHandler) });
extend([
	Polygon,
	Polyline,
	Line
...
//#endregion
export { PointSelectHandler, SelectHandler };